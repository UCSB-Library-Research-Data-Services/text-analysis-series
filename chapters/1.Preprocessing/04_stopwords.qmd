...
Now, let’s return to the worksheet and see how we can put that into practice. We will call `data("stop_words")` to load a built-in dataset from the tidytext package containing common stop words—words like “the”, “and”, and “is” that add little meaning in text analysis. In the next line, `comments_tokens %>% anti_join(stop_words, by = "word")`, will remove any rows in comments_tokens where the word column matches a stop word.
