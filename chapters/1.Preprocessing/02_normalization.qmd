...
Oh, no, if we look at row 1756 we will notice that we still have a waffle 🧇 emoji in that comment. Well, not too worry, while this emoji wasn't included in the original dictionary we used, we can still add it to it for automatic handling:
...
Identify any remaining emoji that wasn’t converted and add it to the emoji dictionary.

add_row(emoji = "🥹", name = "pleading face")
:::

Neat! Let's re-run the code chunk and check it once again. With emojis taken care of, we can now apply the last normalization step

### 11. Dealing with Extraspaces

After completing several normalization steps, we should also account for any extra spaces at the beginning or end of the text. These often come from inconsistent user input, copy-pasting, or formatting issues on different platforms. Let’s clean up these spaces before moving on to the next preprocessing step: tokenization.
