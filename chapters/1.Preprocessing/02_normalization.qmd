...
We won’t have time to cover regular expressions in detail during this workshop, but it’s important to know that they are a fundamental tool for working with text data. Regex allows you to identify, extract, and manipulate patterns in text, such as emails, hashtags, or special characters, which is essential for cleaning and preparing data for analysis. Even though we won’t dive into the syntax or practice exercises here, being aware of regex and its capabilities can help you understand how text preprocessing works behind the scenes and guide you toward resources to learn it on your own.

To make things easier, we’ve pre-populated the `worksheet.qmd` with regex examples and will provide a clear explanation of how they are expected to function, so you can follow along confidently.
...
## 9. Cleaning Punctuation, Symbols and Numbers
...
## 10. Handling Extra Spaces & Elongation

We’ve replaced certain characters with spaces, and some comments already contained extra spaces. To handle this, we will first, use the squish function:

``` r
str_squish() %>% 
```

Additionally, in user-generated content, it’s common to see repeated letters used for emphasis (e.g., “Amaaaazing,” “Loooove”). For that, we have the regex pattern `repeated_chars <- "(.)\1{2,}"`, where:
...
Take a look at the emoji dictionary we loaded into our RStudio environment. It’s packed with more emojis and some surprising meanings than you might expect. We will then write a separate function to deal with those emojis:
...
Wait, we are not done yet, since we need to add the replace_emojis function, based on our loaded dictionary, into our code chunk. This will replace the emojis with their corresponding text:
...
Oh, no, if we look at row 1756 we will notice that we still have a waffle 🧇 emoji in that comment. Well, not too worry, while this wasn't included in the dictionary we used, we can still add it to it for automatic handling:
...
Neat! Let's re-run the code and check it once again.
