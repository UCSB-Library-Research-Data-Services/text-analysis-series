...
Time to launch RStudio and our example! Click on this [link](add%20link) to download the `tdm-workshop.Rproj` file, which includes a `comments.csv` dataset, a quarto (learn more about [Quarto](https://quarto.org/)) `worksheet.qmd` where we will be performing some coding, and an `renv.lock`(learn more about [Renv](https://rstudio.github.io/renv/articles/renv.html)) file listing all the R packages (and their versions) we’ll use during the workshop. This setup ensures a self-contained environment, so you can run everything needed for the session without installing or changing any packages that might affect your other R projects.

### Loading Packages & Inspecting the Data

Open the `worksheet.qmd` file and let's start by loading all the required packages that are pre-installed in the project:

``` r
library(tidyverse)    # general data manipulation and visualization
library(tidytext)     # tokenization and tidy text processing
library(stringr)      # string manipulation (punctuation, whitespaces, etc.)
library(stringi)      # emoji handling
library(dplyr)        # data wrangling
library(textclean)    # Expand contractions, remove URLs/punctuation
library(emo)          # Detect, extract, or replace emojis in text
library(textstem)     # lemmatization; reduce words to dictionary form
```
