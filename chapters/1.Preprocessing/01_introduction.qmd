...
In case you are working with the newest R version, you might experience an error with the installation. To resolve this, search for "matrix" in the `renv.lock` file, manually change its version to 1.7-4, and save the file. Then, re-run `renv::restore()` in your console.
:::
...
Which should show our dataset contains 5877 comments and two columns:

![](images/output-readingdata.png){width="774"}

You’ll notice that we’ve pre-populated some chunks below to save you from the tedious typing. Don’t worry about them for now, we’ll come back to them shortly.
