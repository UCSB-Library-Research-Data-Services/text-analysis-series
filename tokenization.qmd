...
![Image from: OpenAI Community Forum \[829618\]](chapters/1.Preprocessing/images/chat-strawberry.jpeg)
...
Let's go back to our dataset and apply tokenization:

``` r
# Read and check normalized dataset
normalized <- read_csv("normalized.csv")

head(normalized$text)

# Let's make sure we read normalized.csv with correct column types
...
# Tokenize text (transform text data from “wide” or sentence-level form into “long” word-level form)
tokens <- unnest_tokens(data, word, text_noemoji)
```

We’ve just tokenized each comment (or row of text) in our dataset into individual tokens, giving us a total of 124,864 tokens!
